...
export class Resource {
  ...
  $getHeaders(methodOptions?: ResourceActionBase): any | Promise<any> {
    const headers = this.$headers || this.$_getHeaders(methodOptions) || ResourceGlobalConfig.headers || {};
  ...
  $createModel(): ResourceModel<any> {
    const ret = this.$initResultObject();
    ret.$resource = this;
    return ret;
  }
  ...
  protected $request(req: Request, methodOptions: ResourceActionBase = {}): Observable<any> {
    ...
    const requestObservable = this.http.request(req);
  ...
  private $_prepareDataParams(shell: IResourceActionShell) {
    ...
    if (!Array.isArray(shell.data) || shell.params) {
      ...
      for (let i = 0; i < pathParams.length; i++) {
        ...
        const isGetOnly = pathKey[0] === ':';
        if (isGetOnly) {
          pathKey = pathKey.substr(1);
        }

        const value = this.$_getValueForPath(pathKey, shell.extraOptions.params, shell.params || shell.data, usedPathParams);
    ...
    for (const key in shell.extraOptions.params) {
  ...
  private $_prepareSearch(shell: IResourceActionShell) {
    ...
    if (!shell.params) {
      for (const key in shell.searchParams) {
        if (shell.searchParams.hasOwnProperty(key) && !shell.usedPathParams[key]) {
          const value: any = shell.searchParams[key];
  ...
  private $_createMainObservable(shell: IResourceActionShell, requestObservable: Observable<Response>): Observable<any> {
    ...
    return Observable.create((subscriber: Subscriber<any>) => {
      ...
      const reqSubscr: Subscription = requestObservable.subscribe(
        (resp: any) => {
