...
export interface ICourseDetailQuery extends IQuery {
  userId?: number;
  courseId?: number;
  searchYear?: number;
  searchMonth?: number;
...
@Injectable()
@ResourceParams({
  url: `${BASE_URL}/course_detail`
})

// 声明这个class叫TeacherService，名称自取
export class CourseDetailService extends Resource {
  ...
  get: ResourceMethod<{ id: any }, ICourseDetail>;

  @ResourceAction({
    method: RequestMethod.Patch
  })
  query: ResourceMethod<ICourseDetailQuery, QueryResults<ICourseDetail>>;

  @ResourceAction({
    method: RequestMethod.Post
  })
  create: ResourceMethod<ICourseDetail, ICourseDetail>;

  @ResourceAction({
    method: RequestMethod.Put,
    path: '/{!id}'
  })
  update: ResourceMethod<ICourseDetail, ICourseDetail>;
  ...
  removeByDate: ResourceMethod<ICourseDetailQuery, any>;
}
