...
export interface IDistinguishQuery extends IQuery {
  id?: number;
  cd?: string;
  cdName?: string;
  tpName?: string;
...
@Injectable()
@ResourceParams({
  url: `${BASE_URL}/distinguish`
})

// 声明这个class叫TeacherService，名称自取
export class DistinguishService extends Resource {
  ...
  get: ResourceMethod<{ id: any }, IDistinguish>;

  @ResourceAction({
    method: RequestMethod.Patch
  })
  query: ResourceMethod<IDistinguishQuery, QueryResults<IDistinguish>>;

  @ResourceAction({
    method: RequestMethod.Post
  })
  create: ResourceMethod<IDistinguish, IDistinguish>;

  @ResourceAction({
    method: RequestMethod.Put,
    path: '/{!id}'
  })
  update: ResourceMethod<IDistinguish, IDistinguish>;
