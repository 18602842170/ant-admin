...
export interface IRollCallQuery extends IQuery {
  userId?: number;
  courseId?: number;
  userName?: string;
  searchYear?: number;
...
@Injectable()
@ResourceParams({
  url: `${BASE_URL}/roll_call`
})

// 声明这个class叫TeacherService，名称自取
export class RollCallService extends Resource {
  ...
  get: ResourceMethod<{ id: any }, IRollCall>;

  @ResourceAction({
    method: RequestMethod.Patch
  })
  query: ResourceMethod<IRollCallQuery, QueryResults<IRollCall>>;

  @ResourceAction({
    method: RequestMethod.Post
  })
  create: ResourceMethod<IRollCall, IRollCall>;

  @ResourceAction({
    method: RequestMethod.Put,
    path: '/{!id}'
  })
  update: ResourceMethod<IRollCall, IRollCall>;
